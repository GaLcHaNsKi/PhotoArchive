...
describe("category routes", () => {
  test("GET /api/v1/categories returns categories", async () => {
    categoryService.listAll = (async () => [{ id: "cat-1", name: "History", slug: "history", _count: { albums: 5 } }]) as unknown as typeof categoryService.listAll;
    ...
    expect(await response.json()).toEqual({
      items: [{ id: "cat-1", name: "History", slug: "history", _count: { albums: 5 } }]
    });
  });

  test("POST /api/v1/categories creates category for admin", async () => {
    categoryService.create = (async () => ({ id: "cat-1", name: "History", slug: "history" })) as unknown as typeof categoryService.create;

    const app = createApp();
    const response = await app.request("/api/v1/categories", {
      method: "POST",
      ...json({ name: "History", slug: "history" }),
      headers: withCsrf({
    ...
    expect(await response.json()).toEqual({ category: { id: "cat-1", name: "History", slug: "history" } });
  });
});
